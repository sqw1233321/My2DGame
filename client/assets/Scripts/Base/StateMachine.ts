...
import { EntityStateEnum, FsmParamTypeEnum, StateMachineType } from "../Enum";
...
export default abstract class StateMachine extends Component {
  ...
  protected _stateMachines: Map<string, SubStateMachine | State> = new Map();
  protected _animationComponent: Animation;
  protected _type: StateMachineType;
  protected _finishCb: () => void;

  getParams(paramName: string) {
    if (this._params.has(paramName)) {
      return this._params.get(paramName).value;
    }
  }
